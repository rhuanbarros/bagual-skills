...
**Goal:** Run Playwright E2E tests against a fully running stack. Capture browser console logs and server logs. Run visual validation via AI-mediated script (Kimi Code by default) that returns text results — no image processing required from the Harness. Return a structured pass/fail report to the orchestrator.

**Your Role:** Isolated E2E execution agent. Verify the stack is up, instrument logging, run tests, capture everything, delegate screenshot evaluation to `scripts/validate-visual.sh`, read text results. Return results — do NOT fix code.
...
<workflow>
  ...
  <!-- ==================== STEP D: Visual Validation (AI-mediated script) ==================== -->
  <step name="D" goal="Evaluate captured screenshots via validate-visual.sh — always delegates to external vision AI, returns text results">
    <check if="{visual_validation_enabled} == false">
    ...
    <check if="{visual_validation_enabled} == true">
      <output>[E2E Step D] Running AI-mediated visual validation (script mode)...</output>
      ...
      <action>
        *** VISUAL AI PROVIDER DETECTION ***
        Determine which vision AI provider to use. Priority order:

        1. KIMI_API_KEY env var set → use --provider kimi (DEFAULT, always preferred)
        2. OPENROUTER_API_KEY env var set → use --provider openrouter
        3. GEMINI_API_KEY or GOOGLE_API_KEY env var set → use --provider gemini
        4. ANTHROPIC_API_KEY env var set → use --provider claude
        5. OPENAI_API_KEY env var set → use --provider openai
        6. VISUAL_AI_PROVIDER env var explicitly set → use that provider
        7. NONE of the above → HALT with clear instructions to set KIMI_API_KEY

        Set {detected_provider} and {detected_api_key_env} from the first match.
        *** END PROVIDER DETECTION ***
      </action>

      <check if="no API key found for any provider">
        <output>
          ❌ [E2E Step D] HALTED — No vision AI API key configured.

          Visual validation requires an external vision-capable AI. Set ONE of:
            export KIMI_API_KEY="sk-kimi-..."       (recommended — Kimi Code K2.5)
            export OPENROUTER_API_KEY="sk-or-..."    (Open Router)
            export GEMINI_API_KEY="..."              (Google Gemini Vision)
            export ANTHROPIC_API_KEY="..."           (Anthropic Claude Vision)
            export OPENAI_API_KEY="..."              (OpenAI GPT-4V)

          The test pipeline cannot continue without visual validation.
        </output>
        <action>HALT — return failure to orchestrator with clear key-config message</action>
      </check>

      <output>[E2E Step D] Visual AI provider: {detected_provider} (auto-detected from env)</output>

      <!-- ── RUN SCRIPT ─────────────────────────────────────────────────── -->

      <action>
        Run the visual validation script. This delegates ALL image processing
        to the external vision AI — the Harness agent only reads text results.

        Script: `.github/skills/bagual-test-pipeline/scripts/validate-visual.sh`

        ```bash
        bash .github/skills/bagual-test-pipeline/scripts/validate-visual.sh \
          --screenshots-dir {screenshots_dir} \
          --requirements {project_root}/visual-requirements.yaml \
        ...
        The script:
        - Reads each PNG from {screenshots_dir}
        - Looks up requirements in visual-requirements.yaml
        - Sends each screenshot (base64) + evaluation prompt to the vision AI
        - Writes structured JSON to {visual_report}
        - Prints a text summary to stdout
      </action>

      <check if="script failed (exit code != 0)">
        <output>
          ⚠️  [E2E Step D] Visual validation script failed (exit code: {script_exit_code}).
          Stderr: {script_stderr}

          Marking all screenshots as inconclusive — visual validation will show as "script error".
          Check API key and provider configuration.
        </output>
        <action>Set {visual_passed} = true (soft-fail — script error is not a code bug)</action>
        <action>Set {visual_failures} = []</action>
        <action>Set {visual_script_error} = true</action>
      </check>

      <check if="script succeeded">
        <output>[E2E Step D] AI-mediated visual validation complete. Reading results...</output>

        <action>
          Read the JSON report at {visual_report}.
          This file contains TEXT — no image processing needed by the Harness.

          Parse:
          - totalScreenshots, passedCount, failedCount
          - results[].screenshot, results[].passed, results[].issues, results[].description

          Set {visual_results} = the results array.
        </action>

        <check if="failedCount == 0">
          <action>Set {visual_passed} = true and {visual_failures} = []</action>
          <output>[E2E Step D] ✓ All {totalScreenshots} screenshot(s) passed visual validation (provider: {detected_provider}).</output>
        </check>

        <check if="failedCount > 0">
          <action>Set {visual_passed} = false</action>
          <action>Set {visual_failures} = list of { screenshot, issues, description } for each failed result</action>
          <output>[E2E Step D] ❌ {failedCount}/{totalScreenshots} screenshot(s) failed visual validation (provider: {detected_provider}).</output>
        </check>
      </check>

      <check if="{visual_script_error}">
        <output>[E2E Step D] ⚠️  Visual validation: script error — see report for details.</output>
      </check>
    </check>
  </step>
