...
Automated visual validation via **AI-mediated script** (`scripts/validate-visual.sh`). Screenshots are sent to an external vision-capable AI (Kimi Code K2.5 by default) which evaluates them against `visual-requirements.yaml` and returns structured text results. The Harness agent reads ONLY text — zero image processing required.

**Default provider:** Kimi Code K2.5 (Anthropic Messages-compatible API, vision-enabled, 256K context).
Fallbacks: Open Router → Gemini Vision → Claude Vision → GPT-4V.
...
E2E Tests (Playwright)
    │
    ├─ captureScreenshot(page, "name")  →  test-artifacts/screenshots/*.png
    │
    ▼
validate-visual.sh
    │
    ├─ Lê visual-requirements.yaml
    ├─ Para cada PNG: base64 + prompt de avaliação
    ├─ Chama API externa de visão (Kimi Code / Open Router / Gemini / Claude / OpenAI)
    ├─ Parseia resposta (PASS/FAIL + descrição textual)
    └─ Escreve latest.json (TEXTO PURO)
    │
    ▼
Harness (QUALQUER modelo, até text-only)
    │
    └─ Lê latest.json → analisa pass/fail → reporta resultado
```

**Garantia:** O Harness NUNCA processa imagens. Toda avaliação visual é delegada ao script,
que usa uma API externa com capacidade de visão. O Harness só lê JSON.

---

## Provider Priority

| Prioridade | Provider | Modelo Padrão | API Key Env |
|---|---|---|---|
| 1 (default) | Kimi Code | `kimi-k2.5` | `KIMI_API_KEY` |
| 2 | Open Router | `anthropic/claude-sonnet-4-20250514` | `OPENROUTER_API_KEY` |
| 3 | Gemini | `gemini-2.5-flash` | `GEMINI_API_KEY` |
| 4 | Claude | `claude-sonnet-4-20250514` | `ANTHROPIC_API_KEY` |
| 5 | OpenAI | `gpt-4o` | `OPENAI_API_KEY` |

### Configuração mínima (Kimi Code)
