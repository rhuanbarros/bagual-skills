...
**Goal:** Process a single story through the full pipeline: create-story → dev-story → code-review correction loop → mark done → update history → Ledger recording → git commit → Ticket recording.
...
> ### 🔴 ANTI-ORPHAN RULE — every Agent spawn below is SYNCHRONOUS (`run_in_background: false`)
>
> **Harness invariant:** a subagent is NEVER re-invoked when its OWN background children finish —
> only the top-level session receives background-completion notifications. So if you (the
> orchestrator, which may itself have been launched as a background Agent) spawn a child with the
> Agent tool's default (`run_in_background: true`) and then end your turn to "wait for it", you
> ORPHAN: your turn ends, your task flips to `completed`, and nothing ever resumes you — the child
> notifies the top-level session, not you. This has bitten the pipeline twice (create-story /
> dev-story / code-review left mid-flight, stories stuck in `review`).
>
> **THEREFORE:** every `Spawn an Agent subagent` action in this file MUST pass
> `run_in_background: false`. A synchronous spawn BLOCKS your current turn and returns the
> subagent's result INLINE (no turn-ending, no notification dependency), so the chain works
> identically whether you were launched in the foreground or as a background Agent. NEVER spawn a
> child in the background and then wait for a marker file / notification — that is the exact orphan
> trap. If you need results from N children, spawn them synchronously one after another (serialize);
> do not parallelize via background + wait.

> ### 🔴 CLOSED SCOPE — mandatory in every story dispatch generated by this file
>
> **Origin incident (Epic 18, 2026-07-23, `TCK-20260723153000-9f21`):** the same story
> was implemented twice because a `bagual-epic-runner` for a whole epic was
> paused and was resumed with a plain "continue" while, in parallel, dispatches for
> individual stories of THAT SAME epic were already running outside it — neither side
> knew about the other. The root cause was not a tooling issue, it was a prompt-contract
> issue.
>
> **Canonical source (do not duplicate, minimal summary below):**
> `project_controll/gerente/dispatch-contract.md` § "Escopo fechado no prompt de despacho
> paralelo". Every story-dispatch prompt THIS FILE generates — the `Spawn an Agent
> subagent` calls for create-story/dev-story/code-review above and below, and any
> dispatch of the "this story" unit made directly by an external orchestrator (e.g. the
> Gerente dispatching "implement Story X" without going through the whole
> `bagual-epic-runner`) — carries these 5 points:
> 1. **Explicit CLOSED scope**: "deliver ONLY this story, then STOP and return" —
>    never leave it implicit that the agent may continue to the epic's next stories.
> 2. **Declare the sibling agents running NOW and their territories** (files/
>    directories/worktree of each one).
> 3. **Forbid work at the root when worktrees are in use** — someone else's
>    uncommitted file at the shared checkout's root: do not touch, report it.
> 4. **When resuming a paused agent, reaffirm the exact scope** — never just "continue".
> 5. **Never two live orchestrators for the same epic**: a `bagual-epic-runner` for a
>    whole epic and dispatches by individual story of the SAME epic never run at the
>    same time — pick one of the two modes per epic.
...
<workflow>
  ...
  <!-- ==================== SUB-STEP D.6: Ledger recording on completion (E6.6) ==================== -->
  <step name="D.6" goal="Emit Ledger entries for this story's completion, per on-complete-contract.md, generalizing E4.5's wiring beyond bmad-retrospective">
    ...
    <action>This step is ADDITIVE to Step D.5 — D.5 keeps writing the 4 legacy knowledge files unchanged (still the source those files' consumers read today). D.6 generalizes the `on_complete` mechanism that Story E4.5 wired ONLY into `bmad-retrospective` (epic-level) down to this story-level completion point, per `wiki/ledger/on-complete-contract.md` §7 ("Skills de execução em geral... é o trabalho da Story E6.6"). This step REUSES that contract verbatim — it does not redefine schema, grammar, or criteria.</action>

    <action>Read {story_file} (Change Log, Dev Agent Record, Review Findings) and everything reviewed in Step D.5. Classify each candidate insight against on-complete-contract.md §2 (not every conclusion emits): only emit for an item that generalizes beyond this one story — a decision between alternatives that holds going forward (`decisão-técnica`/`decisão-de-arquitetura`), a product-visible behavior decision (`decisão-de-produto`), an actionable/verifiable convention (`regra`), a repeated-≥2x consolidatable way of doing something (`padrão`), or a recurring gotcha with a known fix (`anti-pattern`). When in doubt, do NOT emit — same strict default as the contract (§2: "o custo de perder uma lição pontual é menor que o custo de poluir o Ledger").</action>
    ...
    <check if="at least one item classified Ledger-worthy">
      ...
      <output>[Step D.6] Ledger: {N} entry(ies) emitted (estado: candidata) for {story_key}: {{for each: "- {tipo} candidata: {path}"}}</output>
    </check>

    <check if="zero items classified Ledger-worthy">
      <output>[Step D.6] Ledger: no entry emitted for {story_key} (no item classified as Ledger-worthy — execution only, no new generalization).</output>
    </check>
  ...
  <!-- ==================== SUB-STEP F.5: Ticket recording on completion (E6.6) ==================== -->
  <step name="F.5" goal="If this epic traces back to a bagual-tickets Ticket, record this story's commit hash in that Ticket's ## Fechamento — non-blocking, per E5.4's contract">
    ...
    <check if="no origem: TCK-* found for this epic">
      <output>[Step F.5] Ticket: no traceable origin Ticket for epic {epic_num} — nothing to record. (Not an error: not every epic originates from a Ticket, PRD 02 FR-4 does not require it.)</output>
    </check>
    ...
    <check if="one or more origem: TCK-* ids found">
      ...
      <output>[Step F.5] Ticket: hash {story_commit_hash} recorded in ## Fechamento of {{each TCK-id found}}.</output>
    </check>
  </step>
