...
**Recomendação default**: error-analysis-first. Se o usuário tá começando do zero sem código, faça o setup mínimo (instrument + 5 goldens manuais), gere traces, **e depois** rote pra `deepeval-error-analysis` pra construir os evals reais.
...
A skill `deepeval-error-analysis` é o caminho pra construir evals product-specific — recomenda fortemente que o usuário faça o trace review depois de instrumentar.
...
Sprint 1 (essa semana):
  - Setup DeepEval no projeto (skill: deepeval-setup)
  - Instrumentar agent com @observe (skill: deepeval-instrument)
  - Criar 5-10 goldens manualmente baseados nos failure_examples (skill: deepeval-build-dataset)

Sprint 2 (próxima):
  - **Trace review (open coding) + axial coding** (skill: deepeval-error-analysis) ← CRITICAL
  - Construir judges product-specific calibrados via AlignEval
  - Configurar as 3 métricas do pacote starter (skill: deepeval-pick-metrics)
  - Rodar primeira rodada de evals locais (skill: deepeval-run-and-analyze)
...
Sprint 3+:
  ...
  - Configurar produção via Confident AI com three-tier strategy (skill: deepeval-production)
  - CI/CD com deepeval test run
  - Cadência de calibração biweekly de TPR/TNR dos judges
```

**O passo crítico que a maioria dos times pula é o trace review (deepeval-error-analysis)**. Sem isso, você fica preso em métricas genéricas que não pegam suas falhas reais.
...
### Sprint 1
- [ ] Setup (deepeval-setup)
- [ ] Instrumentação (deepeval-instrument)
- [ ] Dataset inicial com {N} goldens (deepeval-build-dataset)

### Sprint 2
- [ ] Métricas configuradas (deepeval-pick-metrics)
- [ ] Primeira rodada de evals (deepeval-run-and-analyze)
- [ ] Iteração baseada em resultados

### Sprint 3+
- [ ] Expansão de dataset via Synthesizer
- [ ] Métricas custom (GEval)
- [ ] Produção (deepeval-production)
- [ ] CI/CD
...
> "Beleza, plano traçado. O próximo passo é o **Sprint 1**. Quer que eu chame a skill `deepeval-setup` agora pra começar a instalação?"
