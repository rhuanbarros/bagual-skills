...
- [ ] Agent instrumentado com `@observe` (skill `deepeval-instrument`)
- [ ] Dataset com goldens criados (skill `deepeval-build-dataset`)
- [ ] Métricas definidas e configuradas (skill `deepeval-pick-metrics`)
- [ ] Chave LLM configurada (skill `deepeval-setup`)
...
Pra detalhes de CI/CD, veja a skill `deepeval-production`.
...
|-------------------|-------------|------|
| **95%+** | Falsa segurança. Suite não testa coisas que falham. | Volte pro `deepeval-error-analysis`, faça trace review, derive critérios product-specific. |
...
| **<40%** | Definições muito loose OU agent fundamentalmente quebrado | Re-valide as definições dos judges (deepeval-custom-metric). |
...
> "Resultados ótimos! Próximo passo é levar isso pra produção com monitoring contínuo. Quer que eu chame `deepeval-production`?"
...
> "Pelo que vimos, existe um critério não coberto pelas métricas built-in: {coisa}. Pra avaliar isso, vamos criar um GEval custom. Quer que eu chame `deepeval-custom-metric`?"
